...
- **スルーレートリミッタ** — 上昇速い・下降ゆっくりの非対称 RSSI スムージングで TDM ギャップを補完
...
以下の手順でパソコンにビルド環境を構築します。プログラミング初心者の方も順番通りに進めれば問題ありません。

#### ステップ 1 — VS Code をインストール

[**Visual Studio Code**](https://code.visualstudio.com/) を公式サイトからダウンロードし、インストールします（Windows / macOS / Linux に対応）。

#### ステップ 2 — PlatformIO 拡張機能をインストール

1. VS Code を起動します。
2. 左側のサイドバーにある **Extensions（拡張機能）** アイコンをクリックします（または `Ctrl+Shift+X`）。
3. 検索ボックスに **`PlatformIO IDE`** と入力します。
4. 表示された PlatformIO IDE の **「インストール」** をクリックします。
5. インストール完了後、VS Code を**再起動**します。

> 詳しくは [PlatformIO インストールドキュメント](https://docs.platformio.org/en/latest/integration/ide/vscode.html) を参照してください。

#### ステップ 3 — Git をインストール

[**Git**](https://github.com/git-guides/install-git) を公式サイトからダウンロードしてインストールします。

> Windows の場合、インストール中の選択肢はすべてデフォルトのままで問題ありません。

#### ステップ 4 — このリポジトリをクローン

1. VS Code のターミナル（`` Ctrl+` ``）または Git Bash を開きます。
2. 以下のコマンドを実行します：
...
#### ステップ 5 — VS Code でフォルダを開く

VS Code で `ファイル → フォルダーを開く` から、クローンした `PhobosLT_4ch` フォルダを開きます。
PlatformIO が初回起動時に必要なライブラリとツールチェーンを自動でダウンロードします（数分かかる場合があります）。

### 書き込みコマンド

USB でデバイスを接続し、VS Code の PlatformIO ターミナルで実行します：
...
1. ESP32 に書き込み後、Wi-Fi アクセスポイント **`PhobosLT4ch_XXXX`** に接続（XXXX = MACアドレス末尾4文字、パスワード：`phoboslt`）
2. ブラウザで [http://20.0.0.1](http://20.0.0.1) を開く
...
| レース | 設定 | キャリブ |
|---|---|---|
| ![レース画面](images/LapTime.jpg) | ![設定画面](images/cnfig.jpg) | ![キャリブ画面](images/RSSI.jpg) |
...
出口        : filteredRssi が Exit RSSI を EXIT_CONFIRM_SAMPLES 回連続で下回る → ラップ確定
...
- **スルーレートリミッタ** — 上昇最大 SLEW_RISE(10) / 下降最大 SLEW_FALL(1) でTDMギャップのノイズを平滑化
...
| `SLEW_RISE` | `lib/LAPTIMER/laptimer.cpp` | 10 | TDM 1サイクルあたりの最大上昇量 |
| `SLEW_FALL` | `lib/LAPTIMER/laptimer.cpp` | 1 | TDM 1サイクルあたりの最大下降量 |
| `DOMINANCE_DELTA` | `src/main.cpp` | 10 | ドミナンス判定マージン |
| `MAX_PEAK_DURATION_MS` | `lib/LAPTIMER/laptimer.cpp` | 500 | 持続時間ガード(ms) |
| `EXIT_CONFIRM_SAMPLES` | `lib/LAPTIMER/laptimer.cpp` | 1 | 出口確認サンプル数 |
